...
contains
  ...
  function check_explicit_crs(this, row, column) result(explicit)
    ...
    if (this%row_start(row) == 0 .or. this%row_start(row) > size(this%values)) then
      explicit = .false.
      return
    end if
...
!          print*, ii-n_zeroes, this%find_row_by_values_index(ii-n_zeroes)
  ...
  function find_last_index_of_row_crs(this, row) result(row_last_index)
    ...
    else if (this%row_start(row) /= this%row_start(row + 1)) then !This is not the last row and is not all zeroes
